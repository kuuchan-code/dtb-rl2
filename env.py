...
class AnimalTowerDummy(gym.Env):
    ...
    def __init__(self, nanikore=None):
        self.act_num = 22
        self.action_space = Discrete(self.act_num)
        # shape = TRAINNING_IMAGE_SIZE[0] * TRAINNING_IMAGE_SIZE[1]
        self.observation_space = Box(
            low=0, high=255, shape=(3, *TRAINNING_IMAGE_SIZE), dtype=np.uint8)
    ...
    def reset(self) -> np.ndarray:
        ...
        # print(self.blocks)
        self.blocks[self.BLOCKS_HEIGHT_MAX-1] = np.ones(self.act_num)
    ...
    def step(self, action) -> tuple[np.ndarray, float, bool, dict]:
        ...
        reward = 0.0
        if self.each_height[action] >= self.BLOCKS_HEIGHT_MAX - 1:
            done = True
            reward = -1.0
        if self.blocks[self.BLOCKS_HEIGHT_MAX - 2].min() >= 1:
            done = True
            reward = 1.0
    ...
    def get_training_image(self):
        zero_blocks = np.zeros(
            (self.BLOCKS_HEIGHT_MAX, self.act_num), dtype=np.uint8)
        blocks_color = np.stack([zero_blocks, self.blocks, zero_blocks], 2)
        return cv2.resize(blocks_color * 255, dsize=TRAINNING_IMAGE_SIZE[::-1], interpolation=cv2.INTER_NEAREST)
